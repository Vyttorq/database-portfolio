create table Cliente (idCliente int primary key not null, nome varchar (50) not null,
telefone varchar (20) not null, email varchar (50) not null,endereco varchar (100) not
null);
create table Editora (idEditora int primary key not null, nome varchar (50) not null,
telefone varchar (20) not null, email varchar (50) not null, endereco varchar (100) not
null);
create table Livro (idLivro int primary key not null, idEditora int not null, titulo
varchar (100) not null, autor varchar (50) not null,
ano int not null, ISBN varchar(20) not null , preco decimal (5,2), FOREIGN KEY
(idEditora) REFERENCES Editora(idEditora));
create table Estoque (idLivro int not null, Quantidade int not null, FOREIGN KEY
(idLivro) REFERENCES Livro(idLivro));
CREATE TABLE Pedido (idPedido int PRIMARY KEY NOT NULL, idCliente int NOT
NULL, dataPedido date NOT NULL, valorPedido decimal(5,2) NOT NULL, FOREIGN
KEY (idCliente) REFERENCES Cliente(idCliente));
create table ItemPedido (idPedido int not null, idLivro int not null, quantidade int not
null, valorItemPedido decimal (5,2) not null,
foreign key (idPedido) references Pedido(idPedido),
foreign key (idLivro) references Livro(idLivro));
INSERT INTO ItemPedido (idPedido, idLivro, quantidade, valorItemPedido)
SELECT 1, idLivro, 2, preco
FROM Livro
WHERE idLivro = 1;
